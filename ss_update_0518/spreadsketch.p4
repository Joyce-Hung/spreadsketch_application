...
#ifndef SWITCH_ID_VAL
#define SWITCH_ID_VAL 1
#endif
#define BUCKET 32w2048

const bit<8> SWITCH_ID = SWITCH_ID_VAL;
...
control MyIngress(inout headers hdr, 
    ...
    apply {
        if (hdr.ipv4.isValid()) {
            ...
            // s1: forwarding based on haassh routing
            if(SWITCH_ID == 1)
            {
            	// calculate crc32 hash value in switch 
	    	hash(meta.hash_sel, HashAlgorithm.crc32, 8w0, { hdr.ipv4.srcAddr }, 8w255);

            	// choose port based on hash value
            	if (meta.hash_sel % 2 == 0) {
                	standard_metadata.egress_spec = 2;  // toward s2
            	} else {
                	standard_metadata.egress_spec = 3;  // toward s3
            	}
            }
            
            // s2/s3: simply forward to h2/h3
	    else
	    {
