...
void DetectorSS::Report(std::vector<std::pair<key_tp, val_tp> > &result, std::unordered_map<key_tp, myset> &groundmap, double threshold) 
{
	// spreadsketch count
	int cnt = 0;
		...
		if (it->second.size() > threshold) {
                ...
                for (auto res = result.begin(); res != result.end(); res++) {
                    if (res->first == it->first) {
                        key_tp ip = it -> first;
            		std::cout << "Superspreader IP: " << ((ip >> 24) & 0xFF) << "." << ((ip >> 16) & 0xFF) << "." << ((ip >> 8) & 0xFF) << "." << (ip & 0xFF) << " ; " << "Estimated Count: " << (int) it->second.size()<<  " ; " << "True Count: " << (int) res->second << std::endl;
                    }
                }
            }
              
        }
        std::cout << "superspreader check count: "<< cnt << std::endl;
